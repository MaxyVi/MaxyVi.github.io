...
class Root {
    constructor(x, y){
        ...
        this.maxSize = Math.random() * 7 + 30;
        this.size = 0;
        this.vs = Math.random() * 0.2 + 0.5;
        this.angleX = Math.random() * 6.2;
        this.vax = Math.random() * 0.6 - 0.6;
    ...
    update(){
        ...
        this.angle += this.va;
        if (this.lightness < 70) this.lightness += 0.15;
        if (this.size < this.maxSize){

            ctx.save();
            ctx.translate(this.x, this.y);
            ctx.rotate(this.angle);
            ctx.fillRect(0 - this.size/2, 0 - this.size/2, this.size/2, this.size/2);
